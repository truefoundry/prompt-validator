...
from src.common.util.redis_pool_manager import RedisPoolManager
...
class AsyncRedisSaver(BaseCheckpointSaver):
    ...
    @classmethod
    @asynccontextmanager
    async def create(cls) -> AsyncIterator["AsyncRedisSaver"]:
        """Creates a Redis saver instance asynchronously."""
        conn = None
        try:
            conn = await RedisPoolManager().get_redis_connection()
            yield cls(conn)
        finally:
            if conn:
                await conn.close()
