...
def _load_judge_prompt_template() -> str:
    ...
    if _JUDGE_PROMPT_TEMPLATE is None:
        template_path = os.path.join(
            os.path.dirname(__file__), "..", "..", "..", "prompts", "llm_judge_system.txt"
        )
        with open(os.path.normpath(template_path), "r", encoding="utf-8") as f:
            _JUDGE_PROMPT_TEMPLATE = f.read()
...
def build_judge_prompt(metrics: list[str]) -> str:
    ...
    return (
        template
        .replace("__METRIC_BLOCKS__", metric_blocks)
        .replace("__JSON_FIELDS__", json_fields)
        .replace("__EXACT_KEYS__", exact_keys)
    )
